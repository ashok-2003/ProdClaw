# COMPLIANCE CHECK: harness-audit-compliance

## Scope
Workspace quality audit for the compliance agent: contradiction detection + backup diff quality check.

## Agent workspace
`{{OPENCLAW_HOME}}/workspace-compliance/`
...
```
1. Find last backup: {{OPENCLAW_HOME}}/compliance/compliance-backup/agents/compliance/
   → Identify highest version number v<N>
2. Read current files from workspace-compliance/ and backup files from v<N>/
...
- PASS → `harness-audit-compliance — contradiction + backup diff
  Result — PASS
  Output — no contradictions, [N] files changed since last backup: [signal assessment]`
- FAIL → `harness-audit-compliance — contradiction + backup diff
  Result — FAIL
  Contradictions:
