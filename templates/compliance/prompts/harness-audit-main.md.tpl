# COMPLIANCE CHECK: harness-audit-main

## Scope
Workspace quality audit for the main agent: contradiction detection + backup diff quality check.
...
```
1. Find last backup: {{OPENCLAW_HOME}}/compliance/compliance-backup/agents/main/
   → Identify highest version number v<N>
2. Read current files from workspace/ and backup files from v<N>/
...
- PASS → `harness-audit-main — contradiction + backup diff
  Result — PASS
  Output — no contradictions, [N] files changed since last backup: [signal assessment]`
- FAIL → `harness-audit-main — contradiction + backup diff
  Result — FAIL
  Contradictions:
